...
func HightlightSyntaxIn(page: PDFPage, book: String) -> [(
    ...
    let highlightedFragments = monospacedFragments.map { (range, rect) in
        ...
        let highlightedStrings = Highlighter.shared.highlight(str, inBook: book).sorted {
            $0.countForegroundColorAttributes > $1.countForegroundColorAttributes
        }
...
func findMonospacedFragments(page: PDFPage) -> [(NSRange, CGRect)] {
    ...
    func getLines(from characterRanges: [(NSRange, CGRect)]) -> [(NSRange, CGRect)] {
        ...
        for range in sortedRanges {
            if let lastRange = lineRanges.last, abs(lastRange.1.minY - range.1.minY) < 0.39 {
                lineRanges[lineRanges.count - 1].0.length =
                    range.0.location + range.0.length - lineRanges[lineRanges.count - 1].0.location
    ...
    func getMonospacedLines(lineRanges: [(NSRange, CGRect)], characterRanges: [(NSRange, CGRect)])
        -> [(NSRange, CGRect)]
    {
        let sortedRanges = characterRanges.sorted { $0.1.minY < $1.1.minY }
...
func joinContinuousMonospacedLines(in text: NSString, lines: [(NSRange, CGRect)]) -> [(
    NSRange, CGRect
)] {
