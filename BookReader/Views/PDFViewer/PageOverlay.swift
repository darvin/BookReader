...
@objc class PageOverlay: UIView {
    ...
    func makeCodeHighlights() {
        ...
        for (highlightedFragment, rangeOnPage, rectOnPage) in highligtedFragments {
            ...
            highlightedFragment.removingGrayForegroundAttributes().enumerateAttribute(
                .foregroundColor,
                in: NSRange(0..<highlightedFragment.length),
                options: .longestEffectiveRangeNotRequired
            ) {
                color,
                range,
                stop in
                let attrRangeOnPage = NSMakeRange(
                ...
                let rectInCodeHighlight = rectInOverlay.convert(
                    from: rectInOverlayBlock,
                    to: CGRect(
                        x: 0,
                        y: 0,
                        width: rectInOverlayBlock.width,
                        height: rectInOverlayBlock.height
                    )
                )
            ...
            makeCodeHighlight(
                pageBounds: rectOnPageBlock,
                code: highlightedFragment,
                colors: colorization,
                mask: mask
            )
    ...
    private func makeCodeHighlight(
        pageBounds: CGRect,
        code: NSAttributedString,
        colors: [(CGRect, UIColor)],
        mask: CGImage
    ) {
    ...
    func doubleTouched(at: CGPoint) {
        if let highlightView = highlightView(at: at) as? ToggablePoppable {
            ...
            makeHighlight(
                ...
                color: UIColor.red,
                popupView: PopupView(
                    frame: CGRect(origin: CGPointZero, size: r.size),
